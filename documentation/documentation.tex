...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{../plots/intrinsic_calibration/gopro3_first_detection.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.8\textwidth]{imgs/drone_detection_bs.png}
    \includegraphics[width=.8\textwidth]{imgs/drone_detection_of.png}
...
In the first step, the script will check the existence of a \texttt{.csv} file with the results of the $\beta$ search, and if it does not exist, it will run the temporal shift estimation algorithm, that is quite time consuming, even if it is heavily parallelized. For this reason, we suggest not to delete the \texttt{.csv} files named \texttt{beta_results_dataset\_X.csv}.
