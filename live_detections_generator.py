...
for i, camera in enumerate(cameras):
    ...
    global_ts = compute_global_time(frame_ids, alpha[0, i], beta[0, i])
    df.loc[df['cam_id'] == i, 'global_ts'] = global_ts
    
df = df.sort_values(by=['global_ts'])
df.to_csv('detections.csv', index=False)

# Function to write dataframe to ROS2 bag
def write_to_bag(dataframe, bag_path='detections_bag'):
    ...
    for _, row in dataframe.iterrows():
        ...
        msg.data = [row['cam_id'], row['detection_x'], row['detection_y']]
        timestamp = int(row['global_ts'] * 1e9)  # Convert seconds to nanoseconds
        writer.write('detections', serialize_message(msg), timestamp)

    writer.close()

# Convert dataframe to a sorted list of rows and write to bag
df_sorted = df.sort_values(by=['global_ts'])
write_to_bag(df_sorted)
